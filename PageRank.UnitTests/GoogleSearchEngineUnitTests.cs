...
{
    public class GoogleSearchEngineUnitTests
    {
        [Fact]
        public async Task ShouldReturnExpectedSearchResults()
        {
            ...
            webScraperFactory.Setup(s => s.Create(SearchEngineType.Google)).Returns(webScraper.Object);

            var sut = new GoogleSearchEngine(searchEngineDataService.Object, webScraperFactory.Object, baseUrl);
        ...
        public async Task ShouldReturnZeroSearchResult()
        {
            ...
            //assert
            webScraperFactory.Verify(f => f.Create(SearchEngineType.Google));
